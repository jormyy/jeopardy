                ...
                return (
                  <div key={i} className="fj-wager-row">
                    <div className="fj-wager-info">
                      ...
                      <span className={`fj-wager-bankroll ${p.score < 0 ? 'neg' : ''}`}>
                        {formatScore(p.score)}
                      </span>
                    </div>
                    ...
                    return (
                      <div key={i} className="fj-adj-row">
                        ...
                        <div className="fj-adj-info">
                          ...
                          <span className={`fj-adj-score ${p.score < 0 ? 'neg' : ''}`}>
                            {formatScore(p.score)}
                          </span>
                          <span className="fj-adj-wager">Wager: {formatScore(wagers[i])}</span>
                          {adj && (
                            <span className={`adj-preview ${adj === 'correct' ? 'preview-pos' : 'preview-neg'}`}>
                              {adj === 'correct' ? `+${formatScore(wagers[i])}` : `-${formatScore(wagers[i])}`}
                            </span>
                          )}
                        </div>
