...
function App() {
    ...
    const applyAdjustments = (adjustments) => {
        ...
        setPlayers((prev) => prev.map((p, i) => ({ ...p, score: p.score + (adjustments[i] ?? 0) })))
    }
    ...
    const handleQuestionClose = (adjustments) => {
        ...
        setUsedTiles((prev) => {
            const next = new Set([
                ...prev,
                `${activeQuestion.categoryIndex}-${activeQuestion.questionIndex}`,
            ])
    ...
    return (
        <div>
            ...
            {phase === 'winner' && <WinnerScreen players={players} onPlayAgain={handlePlayAgain} />}
        </div>
    )
}
